...
对照 [references/wechat-writing-rules.md](references/wechat-writing-rules.md) 检查。**⚠️ 重要：grep 扫完 ≠ 文章干净**。grep 能抓的是词汇级触发，但段落衔接、新概念引入、讲者铺垫识别都是 grep 抓不到的，必须额外做五项通读检查。

**五项通读检查（grep 不可替代，每次写完必做）：**

- **新概念引入检查**：通读时找每一个英文术语、自造词、比喻的**第一次**出现，问"读者不认识这个词的话，文章在这里或临近位置有没有给说明"。业界熟知的（RAG / F1 / Pydantic）不用；自造术语或比喻（滑杆 / 水坝 / 拳头产品）必须一句话说明。如果读者得往下读三段才能 infer 出意思，前面要加一句「关于 X 的 / 用来 Y 的」
...
- **讲者铺垫识别**：grep `先承认 / 不得不说 / 听起来不性感 / 时间关系 / 我知道这听 / 我可能有偏见`，这些是讲者在台上的缓冲，转述到文章里是纯 filler，一律删，直接保留后面的实质内容
- **术语一致性检查**：作为概念锚点反复出现的词（场景 / 画面 / 观察 / 论点 / 细节 / 画像 / 现象 / 时刻），在终稿阶段各自 grep 一遍，统计出现次数，只应活一个。辅助扫描：`场景.*画面 / 画面.*场景 / 观察.*论点 / 细节.*画面` 这些混用对在同一篇文章里不应共存
...
- **冗余「那个叫 X 的 Y」扫描**：grep `那个叫` / `那种叫` / `所谓的` / `一种被称为` / `一个叫做`，装饰性的一律删，直接用术语名做定语
- **抽象名词堆扫描**：grep `的真正门槛` / `的核心挑战` / `的关键在于` / `的本质是` / `随之清晰` / `一目了然` / `跃然纸上`，换成问句或动作陈述（「X 的真正门槛」→「什么时候才该上 X」）
- **路径隐喻扫描**：grep `走这条路` / `走不通` / `绕开` / `踩过` / `下山` / `转弯`，作者自己写时换白描动词（「没走这条路」→「没使用这个方法」）；原讲者引号里的可保留
- **口语形容词扫描**：grep `很吵` / `很糊` / `很乱` / `很死板` / `很硬` / `很油` / `很脆`，描述系统 / 流程 / 指标时换工程词（「很吵」→「噪音很大」；「很糊」→「精度很低」）
- **评价式名词结构扫描**：grep `描述的.{2,10}非常` / `说的.{2,10}很` / `写的.{2,10}挺` / `提出的.{2,10}很` / `构造的.{2,10}很`，改成主谓宾结构（「S 描述的 X 非常 Y」→「S 描述了一个 Y 的 X」）
- **中文行业术语精度扫描**：grep `被造` / `产品被造` / `软件被造` / `报酬数据` / `报酬曲线` / `造出来`（软件语境）。软件=开发、报酬曲线=薪酬曲线
- **英文数字/习语直译扫描**：grep `高 9[^0-9]` / `低垂的果实` / `第一等大事`，本地化为中文惯用表达
- **讲者语气评价扫描**：grep `说得很冷` / `说得很尖` / `讲得很冷` / `一针见血地` / `云淡风轻地` / `半开玩笑地`，去掉作者对讲者语气的评价
- **非必要动作修饰扫描**：grep `直接讲了一句` / `接着讲了一句` / `简单地说` / `缓缓地说` / `接着一口气讲完`，删掉"直接 / 一句 / 简单地" 等修饰副词
- **不必要比喻扫描**：grep `简单得像` / `快得像` / `重得像` / `慢得像`，改中性陈述
- **主观评判补充句扫描**：grep `这点钱不多` / `这不贵` / `这值得` / `这不算` / `这很` 结尾作独立句子，若是作者的主观价值判断则删除

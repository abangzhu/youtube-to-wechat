...
对照 [references/wechat-writing-rules.md](references/wechat-writing-rules.md) 检查。**⚠️ 重要：grep 扫完 ≠ 文章干净**。grep 能抓的是词汇级触发，但段落衔接、新概念引入、讲者铺垫识别都是 grep 抓不到的，必须额外做三项通读检查。

**三项通读检查（grep 不可替代，每次写完必做）：**

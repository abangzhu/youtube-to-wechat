...
### 演示型分享：整类默认配图，别等单句 deixis 触发

如果视频本身是 conference talk + 现场 demo、产品 UI 演示、或讲者全程对着 slides 讲（判断信号：标题 / 简介里有 demo、talk、building、live；讲者反复说「这就是…的样子」「如你所见」「我来演示一下」），这**一整类默认需要 2-4 张图**，不要逐句等 deixis 命中才想起截图——演示型视频的核心论点几乎全在屏幕上，纯文字必然丢信息。在 Step 4 分析内容、Step 5 动笔之前，先把 demo 关键帧、核心 slide（架构图 / 工作原理流程 / 前后效果对比）过一遍，列出配图点和时间戳，再开写并在正文埋 `[图:MM:SS - 描述]` 占位。

**事后补图也走自动截图，别让用户手动截。** 一篇已成稿、当初漏了图的演示型文章被要求补图时：本文件 Section 二（Chrome DevTools MCP）就是为此存在的，直接用它定位时间戳截图。不要只在正文标注「配图建议」让用户自己截，也不要以「我看不到视频画面」为由退场——截图能力 skill 已具备。只有在用户明确说要自己截、或转录无时间戳且 Section 二的三种 fallback 都不可行时，才退回纯文字占位 `[图:MM:SS - 描述]`。
